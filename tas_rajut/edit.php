...
if (!isset($_GET['id'])) { die("ID Produk tidak valid."); }
$id = intval($_GET['id']);
...
if (!$data) { die("Data produk tidak ditemukan di sistem database."); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_stmt = $conn->prepare("UPDATE produk SET nama_barang=?, kategori=?, stok=?, harga_satuan=?, pembuat=?, gambar=? WHERE id=?");
    $update_stmt->bind_param("ssiissi", $nama, $kategori, $stok, $harga, $pembuat, $nama_gambar, $id);
...
<head>
    ...
    <title>Edit Data - TasRajut</title>
...
<body class="relative flex items-center justify-center min-h-screen p-4 overflow-x-hidden md:p-8">
    ...
    <div class="relative z-10 bg-white/95 backdrop-blur-md p-8 md:p-10 rounded-[2.5rem] shadow-2xl border border-orange-100/60 max-w-2xl w-full transition-all duration-300 my-6">
        ...
        <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
            <div class="w-12 h-12 bg-orange-50 rounded-xl flex items-center justify-center text-2xl border border-orange-100">
                ✏️
            </div>
            <div>
                <h2 class="text-2xl font-black text-gray-900 tracking-tight">Perbarui Data Produk</h2>
                <p class="text-xs text-gray-400 mt-0.5">Modifikasi rincian data koleksi tas rajut yang sudah terdaftar di sistem.</p>
            </div>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select name="kategori" class="w-full pl-11 pr-4 py-3 bg-gray-50/50 border border-gray-200 focus:border-orange-500 rounded-xl text-base focus:outline-none transition-colors duration-200 shadow-2xs cursor-pointer appearance-none">
                        <option value="Sling Bag" <?= $data['kategori'] == 'Sling Bag' ? 'selected' : '' ?>>Sling Bag</option>
                        <option value="Tote Bag" <?= $data['kategori'] == 'Tote Bag' ? 'selected' : '' ?>>Tote Bag</option>
                        <option value="Pouch Mini" <?= $data['kategori'] == 'Pouch Mini' ? 'selected' : '' ?>>Pouch Mini</option>
                        <option value="Backpack" <?= $data['kategori'] == 'Backpack' ? 'selected' : '' ?>>Backpack</option>
                        <option value="Indonesia Ethnic" <?= $data['kategori'] == 'Indonesia Ethnic' ? 'selected' : '' ?>>Indonesia Ethnic</option>
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-black tracking-wider text-gray-400 uppercase mb-2">Stok Barang</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="harga_satuan" value="<?= htmlspecialchars($data['harga_satuan']) ?>" required min="0" 
                               class="w-full pl-11 pr-4 py-3 bg-gray-50/50 border border-gray-200 focus:border-orange-500 rounded-xl text-base focus:outline-none transition-colors duration-200 shadow-2xs">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="pembuat" value="<?= htmlspecialchars($data['pembuat']) ?>" required 
                           class="w-full pl-11 pr-4 py-3 bg-gray-50/50 border border-gray-200 focus:border-orange-500 rounded-xl text-base focus:outline-none transition-colors duration-200 shadow-2xs">
                </div>
            </div>

            <div>
                <label class="block text-xs font-black tracking-wider text-gray-400 uppercase mb-2">Gambar Produk</label>
                <div class="flex items-center gap-4 p-4 border border-gray-200 rounded-xl bg-gray-50/50 mb-2">
                    <div class="w-16 h-16 rounded-lg overflow-hidden bg-white border border-gray-200 flex-shrink-0">
                        <?php 
                        $src_gambar = (strpos($data['gambar'], 'http') === 0) ? $data['gambar'] : "uploads/" . $data['gambar'];
                        ?>
                        <img src="<?= htmlspecialchars($src_gambar) ?>" class="w-full h-full object-cover" alt="Current Image">
                    </div>
                    <div class="text-xs text-gray-400">
                        <p class="font-bold text-gray-500">Gambar Saat Ini</p>
                        <p class="truncate max-w-[200px] md:max-w-xs"><?= htmlspecialchars($data['gambar'] ? $data['gambar'] : 'Belum ada gambar') ?></p>
                    </div>
                </div>
                ...
                <div class="w-full p-4 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50 hover:bg-gray-50 transition-colors">
                    ...
                    <p class="text-[10px] text-gray-400 mt-1.5">*Kosongkan jika tidak ingin mengganti gambar produk utama.</p>
                </div>
            </div>
